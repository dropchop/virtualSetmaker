...
the screen->Unreal map is the identity, so UE coordinates equal the Shot
Designer plan's):
...
direction; wall-snapped angle = wall direction).
"""
...
def test_scene_is_not_mirrored_through_the_camera():
    # An actor on the camera's screen-left in Shot Designer must land on the
    # camera's left in Unreal (UE camera-right = forward rotated +90 yaw).
    scene = parse_file(SAMPLE)
    payload = _scene_payload(scene)
    cam = payload["cameras"][0]["keys"][0]
    right = math.radians(cam["rot"][1] + 90.0)
    red = next(a for a in payload["actors"] if a["color"] == "Red")
    rel = (red["loc"][0] - cam["loc"][0], red["loc"][1] - cam["loc"][1])
    side = rel[0] * math.cos(right) + rel[1] * math.sin(right)
    assert side < 0  # negative = camera-left, matching the SD screen
